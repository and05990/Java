...
public class Main extends JFrame{
    ...
    /*   do{
        ...
        frame.getContentPane().setBackground(new Color(175, 211, 226));
        ...
        buttonAdd.addActionListener(e -> {
            try{
                ...
                if(comboBox.getSelectedIndex() == 0){
                    Tecnico tecnico;
                    if(tipo.equals("Elettronica/Automazione") && interno.equals("Interno")) {
                        tecnico = new Tecnico(nome, cognome, cod, anno_di_inizio, ore, false, true);
                    }else if(tipo.equals("Elettronica/Automazione") && interno.equals("Esterno")) {
                        tecnico = new Tecnico(nome, cognome, cod, anno_di_inizio, ore, false, false);
                    }else if(tipo.equals("Informatica/Telecomunicazioni") && interno.equals("Interno")) {
        ...
        table.setDefaultRenderer(Object.class, new DefaultTableCellRenderer(){
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column){
                ...
                if(row % 2 == 0){
                    c.setBackground(new Color(25, 167, 206));
                }else{
        ...
        buttonSave.setBounds(250, 380, 200, 20);
        buttonSave.setBackground(new Color(25, 167, 206));
        frame.add(buttonSave);

        buttonSave.addActionListener(e -> {
            try{
                String fileName = "Salvataggio.txt";
                File file = new File(fileName);
                if(file.exists()){
                    int confirm = JOptionPane.showConfirmDialog(null, "Il file esiste già, vuoi sovrascriverlo?", "Conferma", JOptionPane.YES_NO_OPTION);
                    if(confirm == 1){
                        return;
                    }
                }
                ...
                JOptionPane.showMessageDialog(null, "Errore: impossibile salvare il file");
            }
        });

        //crate a buttonLoad
        JButton buttonLoad = new JButton("Carica");
        buttonLoad.setBounds(490, 380, 200, 20);
        frame.add(buttonLoad);

        //load the Salvataggio.txt file in to the arrayList dipendenti
        buttonLoad.addActionListener(e -> {
            try{
                String fileName = "Salvataggio.txt";
                File file = new File(fileName);
                if(!file.exists()){
                    JOptionPane.showMessageDialog(null, "Errore: il file non esiste");
                    return;
                }
                ...
                while((line = br.readLine()) != null){
                    ...
                    int cod = Integer.parseInt(data[2]);
                    int anno_inizio = Integer.parseInt(data[3]);
                    double ore = Double.parseDouble(data[4]);
                    String ruolo = data[5];
                    String inf = data[6];
                    String inte = data[7];
                    Boolean interno = new Boolean(inte);
                    Boolean informatico = new Boolean(inf);

                    switch (ruolo) {
                        case "Tecnico" -> {
                            Tecnico tecnico = new Tecnico(nome, cognome, cod, anno_inizio, ore, informatico, interno);
                            dipendenti.add(tecnico);
                        }
                        case "Funzionario" -> {
                                Funzionario funzionario = new Funzionario(nome, cognome, cod, anno_inizio, ore);
                                dipendenti.add(funzionario);
                        }
                        case "Dirigente" -> {
                            Dirigente dirigente = new Dirigente(nome, cognome, cod, anno_inizio, ore);
                            dipendenti.add(dirigente);
                        }
                    }
                }
                br.close();
                fr.close();
                JOptionPane.showMessageDialog(null, "File caricato correttamente");
            }
            catch (IOException ex){
                JOptionPane.showMessageDialog(null, "Errore: impossibile caricare il file");
            }

            for(Progetto dipendente : dipendenti){
                if(dipendente instanceof Tecnico){
                    if(((Tecnico) dipendente).isInformatico()){
                        if(((Tecnico) dipendente).isInterno()){
                            tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Informatica/Telecomunicazioni" ,"Interno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                        }
                        else{
                            tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Informatica/Telecomunicazioni" ,"Esterno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                        }
                    }
                    else{
                        if(((Tecnico) dipendente).isInterno()){
                            tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Elettronica/Automazione" ,"Interno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                        }
                        else{
                            tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Elettronica/Automazione" ,"Esterno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                        }
                    }
                }else if(dipendente instanceof Funzionario){
                    tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Funzionario" ,"Interno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                }else if(dipendente instanceof Dirigente){
                    tableModel.addRow(new Object[]{dipendente.getCodice(), dipendente.getNome(), dipendente.getCognome(), dipendente.getAnno_inizio(),"Dirigente" ,"Interno", dipendente.getOre_lavoro(), dipendente.calcolaStipendio()});
                }
            }
        });
